...
async function main() {
  // Static: restored print page must not be empty (regression from b61da7c)
  const rapportPath = path.resolve(
    "app/(pages)/transactions/impressionRapportComptes/page.jsx"
  );
  const rapportBytes = fs.existsSync(rapportPath)
    ? fs.statSync(rapportPath).size
    : 0;
  record(
    "impressionRapportComptes-file-nonempty",
    rapportBytes > 1000,
    `bytes=${rapportBytes}`
  );
  ...
  // 5) /no-access is public
  {
    const res = await page.goto(BASE + "/no-access", { waitUntil: "domcontentloaded" });
    const body = await page.textContent("body");
    record(
      "no-access-public",
      res?.status() === 200 && /acc[eè]s/i.test(body || ""),
      `status=${res?.status()}`
    );
  }

  // 6) impressionRapportComptes requires auth when unauthenticated
  {
  ...
  // 8) Sign-in page loads; responsive check
  ...
  // Clerk SignIn misconfig on `/` (probe may arrive after initial home wait)
  ...
  record(
    ...
    clerkMisconfig,
    clerkMisconfig
      ? "Clerk SignIn catch-all misconfig observed"
      : "no Clerk misconfig signal this run"
  );

  // Admin dead links / authenticated CRUD only checkable when authenticated
  record(
  ...
  const criticalFails = results.filter(
    r =>
      ...
      (r.name.startsWith("redirect-") ||
        ...
        r.name === "impressionRapportComptes-file-nonempty")
  );
